...
const DatePicker = ({
  ...
  useEffect(() => {
    if (clearDate && flat_picker) {
      try {
        if (Array.isArray(flat_picker)) flat_picker[0].clear();
        else flat_picker.clear();
      } catch (error) {
        console.log(error);
      }
    }
  }, [clearDate, flat_picker]);

  useEffect(() => {
    if (!flat_picker) {
      const instance = flatpickr(".unique-form-datepicker", {
        mode: "single",
        static: true,
      ...
      setFlatPicker(instance);
    }
  }, [onSelect, flat_picker]);
  ...
  return (
    <div className={className}>
      ...
      <div className="relative">
        <input
          className="unique-form-datepicker w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
          placeholder="mm/dd/yyyy"
          data-class="flatpickr-center"
          onChange={(e) => {
